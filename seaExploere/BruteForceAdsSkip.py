...
# -------- REWARD CHECK --------
def is_reward_screen(gray):
    best = 0
    for temp in reward_templates:
        val, _ = match(gray, temp)
        best = max(best, val)

    print(f"🎮 REWARD SCORE: {round(best,2)}")
    return best > 0.65
...
def restart_game():
    ...
    # Step 1: Force stop (app close)
    subprocess.run([ADB, "shell", "am", "force-stop", PACKAGE_NAME])

    # Step 2: Wait after closing (IMPORTANT)
...
while True:
    ...
    reward_visible = is_reward_screen(gray)

    print(f"📊 DETECT → spin:{round(val_spin,2)} | prize:{round(val_prize,2)} | tap:{round(tap_val,2)}")

    # -------- STRONG GAME CONFIDENCE --------
    game_confidence = max(val_spin, val_prize, tap_val)

    is_game = (
    reward_visible or
    tap_val > 0.65 or
    val_spin > 0.85 or   # 🔥 strict spin
    val_prize > 0.75
    )
...
# -------- AD DETECTION --------
    if not is_game:
      ad_counter += 1
      print(f"🚨 AD DETECTED ({ad_counter}/{AD_THRESHOLD}) | conf:{round(game_confidence,2)}")
    else:
        if ad_counter > 0:
           ad_counter -= 1
    ...
    if val_spin > 0.85:
        ...
        for i in range(6):
            tap(x, y)
            print(f"🔁 spin click {i+1}/6")
            time.sleep(3)

        print("🔍 searching spin_cut...")
        ...
        for i in range(5):
            ...
            print(f"spin_cut score: {round(val_cut,2)}")
    ...
    if reward_visible and not wait_for_tap:
        print("🎮 ACTION: REWARD → SWIPE ENABLED")
        swiping = True
    else:
        print("⛔ ACTION: SWIPE DISABLED")
        swiping = False
